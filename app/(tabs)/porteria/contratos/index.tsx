...
const IngresosScreen = () => {
  ...
  const fetchIngresos = async () => {
    try {
      ...
      const response = await fetch(
        "https://kleurdigital.xyz/util/ingresos-salidas-contratos/query_mobile.php",
      );
  ...
  const ingresosFiltradas = ingresos.filter((item) => {
    const query = searchQuery.toLowerCase();
    return (
      item.codigo.toLowerCase().includes(query) ||
      item.ci_placa.toLowerCase().includes(query) ||
      item.nombre_descripcion.toLowerCase().includes(query) ||
  ...
  const handleUpdateStatus = async (id: string, tipo: string) => {
    try {
      ...
      const response = await fetch(
        `https://kleurdigital.xyz/util/ingresos-salidas-contratos/ingresoContrato_mobile.php?id=${id}&tipo=${tipo}`,
      );
          ...
          <>
            <View style={styles3.codeContainer}>
              <View style={[styles3.badge, { backgroundColor: "green" }]}>
                <Text style={styles3.badgeText}>AFUERA</Text>
              </View>
            </View>
          </>
        );
  ...
  return (
    <SafeAreaView style={styles2.container}>
      ...
      <View style={styles2.scrollContainer}>
        <FlatList
          data={ingresosFiltradas}
          renderItem={({ item }) => (
            <View style={styles3.card}>
              <View style={styles3.cardHeader}>
                <View style={styles3.codeContainer}>
                  <Text style={styles3.codeText}>{item.ci_placa}</Text>
                </View>
              </View>
              <View style={styles3.infoRow}>
                <Text style={styles3.label}>Nro. de Contrato: </Text>
                <Text style={styles3.value}>{item.codigo}</Text>
              </View>
              <View style={styles3.infoRow}>
                <Text style={styles3.label}>Contrato: </Text>
                <Text style={styles3.value}>{item.titulo}</Text>
              </View>
              <View style={styles3.infoRow}>
                <Text style={styles3.label}>Descripcion: </Text>
                <Text style={styles3.value}>{item.nombre_descripcion}</Text>
              </View>
              <View style={styles3.infoRow}>
                <Text style={styles3.label}>Entrada: </Text>
                <Text style={styles3.value}>{item.fecha_entrada}</Text>
              </View>
              <View style={styles3.infoRow}>
                <Text style={styles3.label}>Salida: </Text>
                <Text style={styles3.value}>{item.fecha_salida}</Text>
              </View>
